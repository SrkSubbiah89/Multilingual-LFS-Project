...
| `gold_isco_code` | string, 4 digits, in the classifier-supported ISCO catalogue | yes | The correct ISCO-08 **unit-group** code for this case, as determined by the process in `gold_label_source`. Must match `^[0-9]{4}$` **and** exist in `backend/rag/load_full_isco.py`'s `_UNIT` catalogue (436 codes this project's classifier can actually predict — independently verified against the official ILO ISCO-08 standard via a 2026-08-12 primary-source cross-check, live-reconfirmed 2026-08-22) — syntactically valid but nonexistent codes like `0000`/`9999` are rejected. See `eval/dev_set_schema.md`'s "Semantic ISCO-08 code validation" section, including the now-resolved 441-vs-436 discrepancy note. |
