...
| `gold_isco_code` | string, 4 digits, real ISCO-08 code | yes | The correct ISCO-08 **unit-group** code for this case, as determined by the process in `gold_label_source`. Must match `^[0-9]{4}$` **and** exist in `backend/rag/load_full_isco.py`'s `_UNIT` catalogue (441 codes) — syntactically valid but nonexistent codes like `0000`/`9999` are rejected. See `eval/dev_set_schema.md`'s "Semantic ISCO-08 code validation" section. |
...
- `gold_isco_code` exactly 4 digits **and** a real code in the ISCO-08 catalogue (see above) — not just format-valid.
