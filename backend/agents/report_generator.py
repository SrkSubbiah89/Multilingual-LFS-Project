...
class ReportGenerator:
        ...
        try:
            ...
            if profile.isco_code:
                try:
                    ...
                    # dataclasses.asdict() already recurses into nested
                    # dataclasses (e.g. sc.violations), so a single call
                    # is sufficient -- a second asdict() on the already-
                    # converted list raised TypeError on every call here,
                    # silently swallowed by the except below, meaning
                    # semantic_coherence was null in every report ever
                    # generated by this method until this fix.
                    semantic_coherence = dataclasses.asdict(sc)
                except Exception:
                    pass
            ...
            record = self._persist(
                db, session_id, session.language, profile, quality, parsed,
    ...
    def _persist(
        ...
        record = SurveyReportRecord(
            ...
            semantic_coherence_json=(
                json.dumps(semantic_coherence) if semantic_coherence is not None else None
            ),
            isic_classification_json=(
                json.dumps(isic_classification) if isic_classification is not None else None
            ),
            isced_classification_json=(
                json.dumps(isced_classification) if isced_classification is not None else None
            ),
        )
        db.add(record)
        db.commit()
    ...
    def _record_to_report(
        ...
        return SurveyReport(
            ...
            semantic_coherence=_load_json(record.semantic_coherence_json),
            isic_classification=_load_json(record.isic_classification_json),
            isced_classification=_load_json(record.isced_classification_json),
        )
