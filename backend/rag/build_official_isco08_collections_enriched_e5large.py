...
Qdrant collections: the combination test. Same enriched embedding_text
as ENRICHED_PROFILE (official ILO Definition + Included-occupations
text), but embedded with intfloat/multilingual-e5-large (1024-dim)
instead of -small (384-dim).

Built to answer one specific question after two independent, full-scale
confirmations of separate gains: does a larger embedding model
(+8.50pp alone) and richer catalogue text (+11.36pp alone) stack when
combined? They plausibly work via different mechanisms -- embedding-
model capacity vs. input-text richness -- so additivity isn't
guaranteed, only worth actually testing.

Deliberately a separate, simple script, same reasoning as every other
profile-specific build script in this project: shares the catalogue
loader (load_enriched_catalogue, now parameterized by target profile)
and the collection-name/embedding-identity single source of truth in
official_isco08_catalogue.py, so this can never silently drift from
what "official_ilo2021_v1_enriched_e5large" means.
...
_EMBED_MODEL_NAME, _EMBED_VECTOR_DIM = embedding_config_for_profile(ENRICHED_E5LARGE_PROFILE)
_EMBED_PREFIX = "passage: "
_EMBED_BATCH_SIZE = 16   # smaller batch than the e5-small builders -- e5-large is a much bigger model
...
def _target_builds(by_level: dict[str, list[OfficialCatalogueRecord]]) -> list[TargetBuild]:
    names = PROFILE_COLLECTION_NAMES[ENRICHED_E5LARGE_PROFILE]
    builds = []
...
def execute_build(
    ...
    records = load_enriched_catalogue(catalogue_path, metadata_path, profile=ENRICHED_E5LARGE_PROFILE)
    ...
    manifest: dict = {
        ...
        "profile": ENRICHED_E5LARGE_PROFILE,
